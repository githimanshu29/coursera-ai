...
const CreateCourseDialog = ({ isOpen, onClose }) => {
  ...
  const [geminiKey, setGeminiKey] = useState(
    !isPro ? (localStorage.getItem("customGeminiKey") || "") : ""
  );
  ...
  const handleSubmit = async (e) => {
    ...
    // Free tier must have a Gemini key
    if (!isPro) {
      if (!geminiKey || geminiKey.trim().length < 10) {
        setError("Please enter your Gemini API key to generate courses on the free tier.");
        return;
      }
      // Save key to localStorage so axios interceptor picks it up
    ...
    try {
      const cid = uuidv4();
      const res = await generateCourseLayoutApi({
        cid,
        ...formData,
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <p style={{ color: "#6b7280", fontSize: "13px", marginTop: "6px" }}>
            {isPro
              ...
              : "Enter your Gemini API key to generate courses."}
          </p>
        </div>
          ...
          {!isPro && (
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              ...
              <p style={{ color: "#6b7280", fontSize: "11px" }}>
                ...
                {" "}· Saved locally, never sent to our servers.
              </p>
            </div>
          )}
